...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
   ...
   $pdo = getConnection();
    ...
    if ($nom !== '') {
        $query .= "nom_clients = :nom, ";
        $params[':nom'] = $nom;
    }
    if ($prenom !== '') {
        $query .= "prenom_clients = :prenom, ";
        $params[':prenom'] = $prenom;
    }
    if ($numero !== '') {
        $query .= "tel_clients = :numero, ";
        $params[':numero'] = $numero;
    }
    if ($code !== '') {
        $query .= "type_clients = :code, ";
        $params[':code'] = $code;
    }
    if ($sexe !== '') {
        $query .= "sexe_clients = :sexe, ";
        $params[':sexe'] = $sexe;
    }
    if ($cni !== '') {
        $query .= "numero_cni_clients = :numero_cni_clients, ";
        $params[':numero_cni_clients'] = $cni;
    }
    if ($residence !== '') {
        $query .= "residence_clients = :residence, ";
        $params[':residence'] = $residence;
    }

    // Mettre à jour l'horodatage
    $query .= "updated_at_clients = NOW() ";
    $query .= "WHERE id_clients = :id";
    $params[':id'] = $id;
        ...
        $_SESSION['successMessage'] = "
          document.addEventListener(\"DOMContentLoaded\", function() {
           ...
           new window.Swal({
               ...
               text: 'Vous pouvez le consulter dans la liste des clients',
               padding: '2em',
           });
       });";
                    ...
                    <div>
                        ...
                        <div class="pt-5">
                            <div class="panel">
                                    ...
                                    ?>
                                    <div class="mb-5">
                                        <h5 class="font-semibold text-lg dark:text-white-light">Vous modifier le client qui a pour nom <b><?php echo htmlspecialchars($client['nom_clients']);?></b> et prénom <b><?php echo htmlspecialchars($client['prenom_clients']);?></b></h5>
                                    </div>
                                    <div class="space-y-4">
                                        <!-- basic -->
                                        <div x-data="form">
                                                ...
                                                <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-2">
                                                    ...
                                                    <div :class="[isSubmitForm1 ? (form1.code ? 'has-success' : 'has-error') : '']">
                                                        ...
                                                        <select id="gridState" class="form-select text-white-dark" x-model="form1.code" name="code">
                                                            <option values="" disabled selected>Sélectionnez le type de client</option>
