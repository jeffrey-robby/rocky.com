...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($nom !== '') {
        $query .= "nom_stocks = :nom, ";
        $params[':nom'] = $nom;
    }

    // Mettre à jour l'horodatage
    $query .= "updated_at_stocks = NOW() ";
    $query .= "WHERE id_stocks = :id";
    $params[':id'] = $id;
                    ...
                    <div>
                        <ul class="flex space-x-2 rtl:space-x-reverse">
                            ...
                            <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
                                <span>Modification du nom d'un stock</span>
                            </li>
                                ...
                                <div class="panel">
                                        <?php
                                          function getStocksById($id) {
                                            $pdo = getConnection();
                                            $query = "SELECT * FROM stocks WHERE id_stocks = :id";
                                            $stmt = $pdo->prepare($query);
                                         ...
                                         $stocks = getStocksById($id);
                                        ?>
                                        <div class="mb-5">
                                            <h5 class="font-semibold text-lg dark:text-white-light">Vous modifier le stock qui a pour nom <b><?php echo htmlspecialchars($stocks['nom_stocks']);?></b></h5>
                                        </div>
                                        <div class="space-y-4">
                                            <!-- basic -->
                                            <div x-data="form">
                                                <form class="space-y-5" @submit.prevent="submitForm1()" x-ref="myForm" action="" method="POST">
                                                    <div :class="[isSubmitForm1 ? (form1.name ? 'has-success' : 'has-error') : '']">
                                                        ...
                                                        <input id="fullName" type="text" placeholder="Entrer le nouveau nom du stock" class="form-input" x-model="form1.name" name="nom" />
