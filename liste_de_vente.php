                ...
                <div class="animate__animated p-6 animate__fadeInDown">
                    ...
                    <div>
                        <ul class="flex space-x-2 rtl:space-x-reverse">
                            <li>
                                <a href="javascript:;" class="text-primary hover:underline">Ventes</a>
                            </li>
                    ...
                    <div x-data="multipleTable">
                        <div class="panel mt-6">
                            <h5 class="md:absolute md:top-[25px] md:mb-0 mb-5 font-semibold text-lg dark:text-white-light">Liste des ventes</h5>
...
function getVentes() {
    ...
    $query = "
        SELECT 
            ...
            personnels.image_personnels, 
            clients.nom_clients, 
            ventes.total_ventes, 
            ventes.reduction_ventes, 
            ventes.numero_facture_ventes, 
            ventes.created_at_ventes,
            dettes_clients.avance_dettes_clients,
            dettes_clients.montant_dettes_clients
        FROM 
            ventes 
        JOIN 
            personnels ON ventes.id_personnels = personnels.id_personnels 
        JOIN 
            clients ON ventes.id_clients = clients.id_clients
        LEFT JOIN 
            dettes_clients ON ventes.id_ventes = dettes_clients.id_ventes
    ";
...
// Utilisation de la fonction pour récupérer toutes les ventes
$ventes = getVentes();
...
?>
                                    <table id="myTable2" class="whitespace-nowrap dataTable-table">
                                        <thead>
                                            <tr>
                                               <th data-sortable="" style="width: 20.4881%;">Nom personnel</th>
                                               <th data-sortable="" style="width: 20.4881%;">Nom client</th>
                                               <th data-sortable="" style="width: 22.8532%;">Avance</th>
                                               <th data-sortable="" style="width: 22.8532%;">Credit</th>
                                               <th data-sortable="" style="width: 22.8532%;">Total de vente</th>
                                               <th data-sortable="" style="width: 22.8532%;">Réduction de vente</th>
                                               <th data-sortable="" style="width: 22.2901%;">Numéro de facture</th>
                                               <th data-sortable="" style="width: 22.2901%;">Date de vente</th>
                                        ...
                                        <tbody>
                                            <?php foreach ($ventes as $row): ?>
                                                <tr>
                                                    <td>
                                                        <div class="flex items-center font-semibold">
                                                            <div class="p-0.5 bg-white-dark/30 rounded-full w-max ltr:mr-2 rtl:ml-2">
                                                                <img class="h-8 w-8 rounded-full object-cover" src="<?php echo htmlspecialchars($row["image_personnels"]); ?>">
                                                            </div><?php echo htmlspecialchars($row["nom_personnels"]); ?>
                                                        </div>
                                                    </td>
                                                    <td><?php echo htmlspecialchars($row["nom_clients"]); ?></td>
                                                    <td><?php echo htmlspecialchars($row["avance_dettes_clients"]); ?> <b>FCFA</b></td>
                                                    <td><?php echo htmlspecialchars($row["montant_dettes_clients"]); ?> <b>FCFA</b></td>
                                                    <td><?php echo htmlspecialchars($row["total_ventes"]); ?> <b>FCFA</b></td>
                                                    <td><?php echo htmlspecialchars($row["reduction_ventes"]); ?> <b>FCFA</b></td>
                                                    <td><?php echo htmlspecialchars($row["numero_facture_ventes"]); ?></td>
                                                    <td><?php echo htmlspecialchars($row["created_at_ventes"]); ?></td>
                                                    <td><?php  echo "<div class=\"flex items-center\">
                                                    
                                                    <button type=\"button\" class=\"ltr:mr-2 rtl:ml-2\" x-tooltip=\"Voir\">
                                                        <a href=\"liste_de_produit_par_vente.php?id=$row[id_ventes]\">
                                                    ...
                                                    <button type=\"button\" x-tooltip=\"Annuler la vente\" style=\"color: red;\" onClick=\"showAlert$row[id_ventes]()\">
                                                    <script>
                                                    async function showAlert$row[id_ventes]() {
                                                        new window.Swal({
                                                            icon: 'warning',
                                                            title: 'Êtes-vous sûr de vouloir annuler cette vente ?',
                                                            ...
                                                            if (result.value) {
                                                                new window.Swal('Supprimer !', 'La vente a été supprimé.', 'success');
                                                                setTimeout(function() {
                                                                    window.location.href = \"supprimer_une_vente.php?id=$row[id_ventes]\";
                                                                  }, 1000);
                                                            }
                                                        });
                                                    }
