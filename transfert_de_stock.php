                ...
                <div class="animate__animated p-6 animate__fadeInDown">
                        ...
                        <div class="mb-6 grid gap-6 xl:grid-cols-1">
                            <div class="panel h-full">  
                                <div class="flex gap-6">
                                    <label for="fromStock" class="font-semibold text-lg dark:text-white-light">Emplacement source :</label>
                                    <select id="fromStock" name="fromStock" required>
                                        <?php foreach ($stocks as $stock): ?>
                                            <option value=""></option>
                                            <option value="<?php echo $stock['id_stocks']; ?>"><?php echo $stock['nom_stocks']; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                    <label for="to_stock" class="font-semibold text-lg dark:text-white-light">Emplacement de destination :</label>
                                    <select id="to_stock" name="to_stock" required>
                                        <?php foreach ($stocks as $stock): ?>
                                            <option value=""></option>
                                            <option value="<?php echo $stock['id_stocks']; ?>"><?php echo $stock['nom_stocks']; ?></option>
                                        <?php endforeach; ?>
                    ...
                    <div id="transferModal" class="modal">
                        <div class="modal-content">
                            ...
                            <h2 class="font-semibold text-lg dark:text-white-light flex gap-2" style="padding-bottom:10px; ">Confirmer le transfert des produits sélectionnés du stock : <p id="selectedStock1" style="color:red;"></p> vers le stock : <p id="selectedStock" style="color:red;"></p></h2>
        ...
        <script>
           ...
           showMessage1 = (msg = 'Example notification text.', position = 'bottom-start', showCloseButton = true, closeButtonHtml = '', duration = 4000) => {
            ...
            document.getElementById('confirmTransferButton').addEventListener('click', function() {
                ...
                const toStockSelect = document.getElementById('to_stock');
                const toStockSelect1 = document.getElementById('fromStock');
                const selectedStockId = toStockSelect.value;
                const selectedStockId1 = toStockSelect1.value;
                const selectedStockName = toStockSelect.options[toStockSelect.selectedIndex].text;
                const selectedStockName1 = toStockSelect1.options[toStockSelect1.selectedIndex].text;
            
                if (!selectedStockId1) {
                    showMessage("Veuillez sélectionner un stock source.", 'top');
                    return;
                }

                if (!selectedStockId) {
                    showMessage("Veuillez sélectionner un stock de destination.", 'top');
                    return;
                }

                if (selectedStockId === selectedStockId1) {
                    showMessage("Le stock de sortie et de destination ne peuvent pas être similaire.", 'top');
                    return;
                }
            
                // Afficher le nom du stock sélectionné
                document.getElementById('selectedStock').textContent = selectedStockName;
                document.getElementById('selectedStock1').textContent = selectedStockName1;
                ...
                checkboxes.forEach(checkbox => {
                   ...
                   // modalRow.insertCell(6).textContent = productId;
                   const idCell = modalRow.insertCell(6);
            ...
            document.getElementById('finalizeTransferButton').addEventListener('click', function() {
                const toStock = document.getElementById('to_stock').value;
                const fromStock = document.getElementById('fromStock').value;
                const transfers = [];
            
                const modalRows = document.querySelectorAll('#modalProductsTable tbody tr');
                modalRows.forEach(row => {
                    //const productId = row.cells[6].textContent;
                    const productId = row.cells[6].getAttribute('data-product-id');
                    const quantityInput = row.cells[5].querySelector('input[type="number"]');
                    const quantity = quantityInput.value ;
            
                    if (quantity > 0) {
                        transfers.push({ id: productId, quantity: quantity });
                    }
                ...
                fetch('transfer.php', {
                    ...
                    body: JSON.stringify({ to_stock: toStock, fromStock: fromStock, transfers: transfers }),
                })
